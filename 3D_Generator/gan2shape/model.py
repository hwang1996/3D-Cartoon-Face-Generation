...
class GAN2Shape():
    def __init__(self, cfgs):
        ...
        # self.generator1 = Generator(self.gan_size, self.z_dim, 8, channel_multiplier=self.channel_multiplier)
        self.generator2 = Generator(self.gan_size, self.z_dim, 8, channel_multiplier=self.channel_multiplier)
        ...
        # ffhq_gan_ckpt = torch.load(cfgs.get('ffhq_gan_ckpt'), map_location=map_func)
        # self.generator1.load_state_dict(ffhq_gan_ckpt['g_ema'], strict=False)
        # self.generator1 = self.generator1.cuda()
        # self.generator1.eval()
        gan_ckpt = torch.load(cfgs.get('gan_ckpt'), map_location=map_func)
        self.generator2.load_state_dict(gan_ckpt['g_ema'], strict=False)
        self.generator2 = self.generator2.cuda()
        ...
        if self.truncation < 1:
            with torch.no_grad():
                # self.mean_latent1 = self.generator1.mean_latent(4096)
                self.mean_latent2 = self.generator2.mean_latent(4096)
        else:
            self.mean_latent2 = None
    ...
    def load_latent(self):
        with torch.no_grad():
            def get_w_img(w_path):
                ...
                swap = True 
                swap_layer_num = 2
                # gan_im1, save_swap_layer = self.generator1([latent_w], input_is_w=True, truncation_latent=self.mean_latent1,
                #                            truncation=self.truncation, swap=swap, swap_layer_num=swap_layer_num)
                # gan_im, _ = self.generator2([latent_w], input_is_w=True, truncation_latent=self.mean_latent2,
                #                            truncation=self.truncation, swap=swap, swap_layer_num=swap_layer_num, swap_layer_tensor=save_swap_layer)
                gan_im, _ = self.generator2([latent_w], input_is_w=True, truncation_latent=self.mean_latent2,
            ...
            self.center_w = self.generator2.style_forward(torch.zeros(1, self.z_dim).cuda())
            self.center_h = self.generator2.style_forward(torch.zeros(1, self.z_dim).cuda(), depth=8-self.F1_d)
    ...
    def init_ellipsoid(self):
        with torch.no_grad():
            ...
            elif self.category in ['face', 'cartoon']:  # celeba
                max_y = h - 1
                width = max_x - min_x
                max_x -= width / 12
                min_x += width / 12
    ...
    def parse_mask(self, image):
        with torch.no_grad():
            ...
            save_image(image[0], 'img1.png')
            if self.category in ['face', 'synface', 'cartoon']:
                out = out[0]
            out = out.argmax(dim=1, keepdim=True)
            if self.category == 'face' or self.category == 'cartoon':
    ...
    def latent_project(self, image):
        ...
        offset = self.generator2.style_forward(hidden, skip=8-self.F1_d) - self.center_w
    ...
    def gan_invert(self, image, batchify=0):
        def gan_invert_sub(image):
            ...
            swap = True 
            swap_layer_num = 2
            # gan_im1, save_swap_layer = self.generator1([latent], input_is_w=True, truncation_latent=self.mean_latent1,
            #                            truncation=self.truncation, swap=swap, swap_layer_num=swap_layer_num)
            # gan_im, _ = self.generator2([latent], input_is_w=True, truncation_latent=self.mean_latent2,
            #                            truncation=self.truncation, swap=swap, swap_layer_num=swap_layer_num, swap_layer_tensor=save_swap_layer)
            gan_im, _ = self.generator2([latent], input_is_w=True, truncation_latent=self.mean_latent2,
    ...
    def save_albedo_results(self, stage=0):
        path = self.image_path
        idx1 = path.rfind('/')
        idx2 = path.rfind('.')
        img_name = path[idx1+1:idx2]
